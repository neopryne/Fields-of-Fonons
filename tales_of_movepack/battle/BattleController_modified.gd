...
func try_remove_effect(effect_name):
	pass
...
func construct_fof_change_table():
	...
	add_fof_change("TO_MOVE_SPLASH_NAME", "", "MOVE_HURRICANE_NAME", "MOVE_TORRENT_NAME", "MOVE_BOIL_NAME")#todo this is default for water moves
	...
	add_fof_change("MOVE_AVALANCHE_NAME", "", "", "MOVE_FOG_NAME", "MOVE_FOG_NAME")
...
func lookup_fof_change(current_move:BattleMove, effect_name:String):
	#Only check effects that are FOFs.
	match effect_name:
		DARK_FOF, EARTH_FOF, AIR_FOF, WATER_FOF, FIRE_FOF, LIGHT_FOF:
			pass
		_:
			return null
	if effect_name == DARK_FOF:
		effect_name = EARTH_FOF if rand.rand_bool() else WATER_FOF
	if effect_name == LIGHT_FOF:
		effect_name = FIRE_FOF if rand.rand_bool() else AIR_FOF
	print("lookup fof change for ", current_move.name, " with ", effect_name)
...
func fof_change_check(current_move:BattleMove, fighter:FighterNode):
	...
	for effect_node in fighter.status.get_effects():
		...
		if new_move != null:
			print("fof change! ", current_move.name, " becomes ", new_move.name)
...
func add_fonic_effect(move:GenericAttack, effect:StatusEffect):
	...
	if (move.status_effects_to_apply == 0):
		...
		if (move.status_effect_chance == 0):
			if (move.cost == 0):
				move.status_effect_chance = 50#todo 25
			elif (move.cost == 1):
				move.status_effect_chance = 75
			else:
